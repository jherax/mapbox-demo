...
const APP_PORT = process.env.EXPRESS_PORT || '9000';
startTask();
...
function startTask() {
  ...
  fs.mkdir(docsFolder, {recursive: true})
    ...
    .then(() => console.info(`File written: ${readmeHtml}`))
    .catch(error => {
      throw error;
    });
}
    ...
    return [
      {
        ...
        filter: source =>
          source
            .replace(/localhost:9000/gi, `localhost:${APP_PORT}`)
            .replace(
              /"(backend|src)\//gi,
              '"https://github.com/jherax/mapbox-demo/blob/master/$1/',
            )
            .replace(
              /(<pre[^>]*>)?[\n\s]?<code([^>]*)>/gi,
